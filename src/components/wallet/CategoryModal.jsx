...
function CategoryModal({ open, onClose }) {
  ...
  const CatList = ({ items, flow, newState, setNew }) => (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14, minHeight: 32 }}>
        ...
        {items.map(c => (
          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 6px', borderRadius: 6,
                                   background: 'var(--bg-inset, var(--bg-app))', minHeight: 30 }}>
  ...
  return (
    <Modal open={open} onClose={onClose} title="Manage Categories"
           subtitle="Add or remove income & expense types used when recording transactions"
