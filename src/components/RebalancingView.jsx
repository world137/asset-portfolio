/* eslint-disable */
/* RebalancingView.jsx — Feature #1: Target Allocation & Rebalancing */
...
function RebalancingView() {
  ...
  const rows = window.ASSET_CLASSES.map(cls => {
    ...
    const delta    = tgtValue - curValue; // positive = need to buy, negative = need to sell
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div>
          <h1 className="t-h1" style={{ margin: '0 0 2px' }}>Rebalancing</h1>
      ...
      {hasTgt && (
        <React.Fragment>
          ...
          <div className="kpis" style={{ marginBottom: 18 }}>
            ...
            <div className="kpi">
              <div className="lab">Overweight Classes</div>
            ...
            <div className="kpi">
              <div className="lab">Underweight Classes</div>
                ...
                <div style={{ display: 'flex', height: 20, borderRadius: 4, overflow: 'hidden', background: 'var(--bg-sunken)' }}>
                  {rows.filter(r => r.tgtPct > 0).map(r => (
                    <div key={r.cls.key} title={`${r.cls.label}: ${r.tgtPct.toFixed(1)}%`}
                         style={{ width: r.tgtPct + '%', background: r.color, transition: 'width 0.3s', opacity: 0.7 }} />
                  ))}
                </div>
              </div>
              {/* Legend */}
          ...
          <div className="card">
            <div className="card-h">
              ...
              <div className="s">Buy/sell amounts to reach target allocation · in {settings.displayCcy}</div>
            </div>
              ...
              <table className="ptable" style={{ minWidth: 680 }}>
                ...
                <tbody>
                  {rows.filter(r => r.curValue > 0 || r.tgtPct > 0).map(r => {
                    ...
                    const driftColor = driftAbs < 1 ? 'var(--fg-3)'
                      : driftAbs < 3 ? 'var(--fg-2)'
                      : r.drift > 0 ? 'var(--red-600)' : 'var(--green-600)';
                        ...
                        <td className="num">
                          {r.tgtPct > 0 && Math.abs(r.delta) > 1 ? (
                            <span style={{
                              ...
                              background: r.delta > 0 ? 'var(--green-bg, rgba(22,163,74,0.1))' : 'var(--red-bg, rgba(220,38,38,0.1))',
