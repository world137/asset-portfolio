...
function HoldingModal({ open, onClose, classKey, lot }) {
  ...
  React.useEffect(() => {
    if (open) {
      if (lot) {
        setF({
          name: lot.name || '', type: lot.type || 'Debenture',
          price: lot.price ?? '', qty: lot.qty ?? '', cur: lot.cur ?? '',
  ...
  const footer = (
    <React.Fragment>
      ...
      <span style={{ display: 'flex', gap: 8 }}>
        ...
        <Button variant="accent" icon={editing ? 'check' : 'plus'} onClick={save} disabled={!valid}>{editing ? 'Save lot' : 'Add lot'}</Button>
      </span>
    </React.Fragment>
  );
    ...
    <Modal open={open} onClose={onClose}
      ...
      <div className="mgrid">
        <div className="full">
          ...
          <input className="input" value={f.name} onChange={set('name')} placeholder={isOther ? 'e.g. BCP292A' : isCrypto ? 'e.g. BTC, ETH, SOL' : 'e.g. AAPL'} autoFocus />
        </div>
        ...
        <div>
          ...
          <datalist id="sectorlist">
            {['Technology','Semiconductor','Financials','Healthcare','Consumer','Energy','Industrials','Materials','Aerospace','S&P 500','Korea','China','Commodity','Infrastructure','Income'].map(s => <option key={s} value={s} />)}
          </datalist>
        </div>
        ...
        <div className="full computed">
          ...
          <div className="c"><small>Unrealized P/L</small><span className={profit >= 0 ? 'up' : 'down'}>{(profit >= 0 ? '+' : '−') + window.fmtMoney(Math.abs(profit), cls.ccy, 2)}</span></div>
        </div>
      </div>
    </Modal>
  );
